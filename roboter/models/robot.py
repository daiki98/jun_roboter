...
class RestaurantRobot(Robot):
    ...
    def __init__(self, name=DEFFAULT_ROBOT_NAME):
        super().__init__(name=name)
        self.ranking_model = ranking.RankingModel()
        
    def _hello_decorator(func):
        """Decorator to say greeting if you are not greeting the user."""
        def wrapper(self):
    ...
    def recommend_restaurant(self):
        """Show restaurant recommended restaurant tu the user."""
        ...
        while True:
            ...
            is_yes = input(template.substitiute({
                'robot_name': self.name,
    ...
    def ask_user_favorite(self):
        """Collect favorite restaurant information form users."""
        while True:
